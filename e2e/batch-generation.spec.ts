...
test.describe('バッチ記事生成ページ', () => {
  ...
  test('ジョブ履歴リストが表示されること', async ({ page }) => {
    ...
    if (!bodyText.trim()) return
    // ジョブ履歴テーブルまたはリスト
    const jobHistory = page.locator('[data-testid="job-history"]')
      .or(page.locator('table'))
      .or(page.getByText('履歴'))
      .or(page.getByText('History'))
  ...
  test('コスト表示エリアが存在すること', async ({ page }) => {
    ...
    if (!bodyText.trim()) return
    // コスト情報の表示
    const costArea = page.locator('[data-testid="cost-display"]')
