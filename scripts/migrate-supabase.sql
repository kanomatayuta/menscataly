...
-- ============================================================
-- 24. Migration 014: article_review_queue.article_id を nullable に変更
-- コンプライアンスゲートは記事公開前に実行されるため、
-- article_id がまだ存在しない状態でキューエントリを作成する必要がある
-- ============================================================

INSERT INTO schema_migrations (version, description) VALUES
  ('014', 'article_review_queue: article_id を nullable に変更（公開前キュー永続化対応）')
ON CONFLICT (version) DO NOTHING;

ALTER TABLE article_review_queue ALTER COLUMN article_id DROP NOT NULL;

-- ============================================================
-- 25. Migration 015: app_config (キー・バリュー設定テーブル)
-- ============================================================

INSERT INTO schema_migrations (version, description) VALUES
  ('015', 'app_config: キー・バリュー設定テーブル（自動化設定等）')
ON CONFLICT (version) DO NOTHING;
...
-- ============================================================
-- Done — v3.1 (Migration 015: app_config)
-- ============================================================
